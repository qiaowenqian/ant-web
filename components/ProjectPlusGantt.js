...
export default class ProjectPlusGantt extends React.Component {
  constructor() {
    super();
    this.state = { gantt: "", topTime: "月", loading: true, taskId: "" }
  }
  ...
  componentDidMount() {
    ...
    const topTime = Storage.getLocal("topTime");
    if (topTime) { this.setState({ topTime: topTime }) }
  ...
  setBoxColor(gantt) {
    ...
    gantt.on("drawitem", function (e) {
      ...
      if (!item.Summary && !item.Milestone) {
        // Summary 是否有子任务 Milestone 工期是否为0
        if (e.baseline) {
          ...
          e.itemHtml =
            '<div id="' +
            item._id +
            '" class="mini-gantt-item" style="top:' +
            top +
            "px;left:" +
            left +
            "px;width:" +
        ...
        if (e.baseline) {
          ...
          e.itemHtml =
            '<div id="' +
            item._id +
            '" class="mini-gantt-item  mini-gantt-summary" style="left:' +
            left +
            "px;top:" +
            top +
            "px;width:" +
            ...
            "px;background:" +
            stateColor +
            '"><div class="mini-gantt-summary-left ' +
            stateBgColor_left +
            '"></div><div class="mini-gantt-summary-right ' +
            stateBgColor_right +
            '"></div></div>';
        }
      }
    ...
    gantt.on("itemtooltipneeded", function (e) {
      ...
      if (e.baseline) {
        ...
        if (task.planFinish) {
          planFinish = that.dateToString(task.planFinish);
        }
        e.tooltip =
          "<div>任务：" +
          task.Name +
  ...
  render() {
    ...
    const content = (
      <div className="taskState">
        ...
        <ul>
          <li>
            <span style={{ background: "#78c06e" }} />
            <font>进行中</font>
          </li>
          <li>
            <span style={{ background: "#ff943e" }} />
            <font>已逾期</font>
          </li>
          <li>
            <span style={{ background: "#5ec9f6" }} />
            <font>待确认</font>
          </li>
          <li>
            <span style={{ background: "#38adff" }} />
            <font>提前完成</font>
          </li>
          <li>
            <span style={{ background: "#5e97f6" }} />
            <font>正常完成</font>
          </li>
          <li>
            <span style={{ background: "#5c6bc0 " }} />
            <font>逾期完成</font>
          </li>
          <li>
            <span style={{ background: "#9a89b9" }} />
            <font>未指派</font>
          </li>
          <li>
    ...
    return (
      <div className="projectPlusGanttBox">
        ...
        <div className="projectPlusGantt">
          <div className="top">
            {/*<Radio.Group value={topTime} size="small" onChange={(e)=>{this.changeTopTimeScale(e.target.value)}}>
            ...
            <div className="RadioGroup">
              <label
                onClick={() => {
                  this.changeTopTimeScale("日");
                }}
                className={topTime === "日" ? "act" : ""}
              >
                日
              </label>
              <label
                onClick={() => {
                  this.changeTopTimeScale("周");
                }}
                className={topTime === "周" ? "act" : ""}
              >
                周
              </label>
              <label
                onClick={() => {
                  this.changeTopTimeScale("月");
                }}
                className={topTime === "月" ? "act" : ""}
              >
                月
              </label>
              <label
                onClick={() => {
                  this.changeTopTimeScale("季");
                }}
                className={topTime === "季" ? "act" : ""}
              >
                季
              </label>
              <label
                onClick={() => {
                  this.changeTopTimeScale("年");
                }}
                className={topTime === "年" ? "act" : ""}
              >
                年
              </label>
            </div>
            <div className="topIcon">
              <Popover trigger="click" content={content} placement="bottomLeft">
                <i className="iconfont icon-xinxitishi" />
              </Popover>
            </div>
          </div>
          <div className="bottom" id="projectPlusGantt" />
        </div>
      </div>
    );
  }
