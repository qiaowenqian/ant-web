...
export default class ProjectSelect extends React.Component {
  ...
  componentWillMount() {
    ...
    if (this.props.selectedProjects) {
      this.props.selectedProjects.map((item, i) => {
        projectSelecteds.push(item);
      });
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.selectedProjects) {
      nextProps.selectedProjects.map((item, i) => {
        projectSelecteds.push(item);
      });
      this.setState({ projectSelecteds: projectSelecteds });
    }
          ...
          <div className="selectContent">
            <div className="contentLeft">
              ...
              <div className="contentList">
                <ul>
                  {projectSelecteds && projectSelecteds.length > 0
                    ? projectSelecteds.map((tim, index) => {
                        return (
                    ...
                    return (
                      <div className="project" key={item.id}>
                        <div
                          className="check"
                          onClick={() => {
                            this.selectedProject(item.id, item.proname);
                          }}
                        >
                          <Checkbox
                            checked={
                              projectSelecteds.filter(val => val.id === item.id)
                                .length > 0
                                ? true
                                : false
                            }
                          ...
                          <span className="projectIcon">
                            {item.attstr04 ? (
                              ...
                              reg2.test(item.attstr04) ? (
                                ...
                                <svg className="svgIcon" aria-hidden="true">
