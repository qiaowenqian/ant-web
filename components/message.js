...
export default class Message extends React.Component {
  ...
  read(item) {
    ...
    updateRead(ids, data => {
      ...
      Router.push(
        "/pc_projectDetails?id=" + item.projectId + "&taskId=" + item.taskinfoId
      );
    });
  }
  ...
  renderMess() {
    ...
    let data =
      messData && messData.length > 0 ? (
        messData.map(function(item, index) {
          let content = {};
          try {
          ...
          for (var o in content) {
            arr.push(
              <span className="tit" style={{ display: "block" }} key={o}>
                ...
                {o == "讨论内容" ? (
                  ...
                  <div className="cont">{content[o]}</div>
                )}
              </span>
            );
          }
            ...
            <li key={index}>
              <div className="mess">
                <div className="messBox">
                  {item.type != "b" ? (
                    <Popover placement="right" content="点击查看任务详情">
                      {item.subject.indexOf("批量修改") === -1 ? (
                        ...
                        <div className="tit-size" style={{ cursor: "pointer" }}>
                          <span className="subject">{item.subject}</span>
                          <span className="time tit">{item.createDate}</span>
                        </div>
                      )}
                  ...
                  <div className={item.read == "0" ? "read readRed" : "read"} />
                </div>
              </div>
              <div className="clear" />
            </li>
    ...
    return (
      ...
      >
        <div className="mes_box">
          ...
          {messageCount > 0 ? (
            <div className={read == "1" ? "tag tag-red" : "tag"}>
