...
export default class dingJS extends React.Component {
  ...
  static selectUser(
    ...
    DingTalkPC.biz.contact.choose({
      ...
      // limitTips: "超出了",
      // maxUsers: 1000, //默认不限制
      // pickedUsers: users, //已选，但可取消，只针对多选生效
      // disabledUsers: users, //不可选，，只针对多选生效
      // requiredUsers: users, //必选，只针对多选生效
  ...
  //Ding
  static DingMessage(usersid, number, taskName, ProjectName, taskId) {
    console.log(usersid, number, taskName, ProjectName);
    let corpid = Storage.get("corpIdWork");
    console.log(DingTalkPC.biz.ding, "ding");
    console.log(DingTalkPC.biz);
    DingTalkPC.biz.ding.post({
      users: usersid, //用户列表，userid
      corpId: corpid, //企业id
      type: 2, //钉类型 1：image  2：link
      alertType: 2,
      alertDate: { format: "yyyy-MM-dd HH:mm", value: "" },
      attachment: {
        title: `${number} ${taskName}`, //附件的标题
        // http://114.116.119.166/antvip/ant-cgi/user/isMobileDeviceNew?corpid=$CORPID$
        url: ` http://47.94.248.148/antvip/ant-cgi/user/isMobileDeviceNew?corpid=${corpid}&taskId=${taskId}`, //附件点击后跳转url

        // url: ` https://ddy.antbim.net/antvip/ant-cgi/user/isMobileDeviceNew?corpid=${corpid}&taskId=${taskId}`, //附件点击后跳转url
        image: "https://ddy.antbim.net/static/react-static/pcvip/imgs/logo.png", //附件显示时的图片 【可选】
        text: ProjectName //附件显示时的消息体 【可选】
      },
      text: "", //消息体
      onSuccess: function() {
        console.log("发送成功");
        message.success("Ding消息发送成功");
      },
      onFail: function() {
        console.log("失败");
      }
  ...
  static uploadImage(onSuccess, multiple = false, isSmall) {
    HttpClient.AjaxPost(
      "/uploadAttachment/getUploadMethod",
      "",
      result => {
        let data = result;
        if (data.state) {
  ...
  /**TODO:预览图片实现多个图片同时预览 */
  static previewImages(files, isSmall, urlList) {
    let corpid = Storage.get("corpidWork");
    if (isSmall) {
      corpid = Storage.get("corpidMsg");
    }
    if (files.fileId) {
      ...
      let suffixIndex = url.lastIndexOf(".");
      let suffix = url.substr(suffixIndex + 1).toUpperCase();
      if (
        suffix != "BMP" &&
        suffix != "JPG" &&
        suffix != "JPEG" &&
  ...
  static authDingJsApi(onSuccess) {
    ...
    HttpClient.AjaxPost(
      "/dingTalk/mobilejs?urlData=" + urlData,
      "",
      result => {
        let data = result;
        DingTalkPC.config({
   ...
   */
  static GetUserDetailInfoPage(id = "", corpId = "", successBack, failBack) {
   ...
   * @param {*目录空间Id} fileId
   */
  static DeleteFileFromDingDing(spaceId, dentryId, successBack, failBack) {
