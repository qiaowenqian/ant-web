...
//编辑标签
export function updateLabel(data, callback, isSmall) {
  HttpClient.AjaxPost(
    "/label/updateLabel",
...
export function updateLabelParent(id, parentId, callback) {
  HttpClient.AjaxPost(
    "/label/updateLabelParent",
    { id: id, parent: { id: parentId } },
    list => {
      callback(list);
    }
  );
...
//根据是不是管理员 获取个人标签
export function findLabelByAdimn(isAdimn, callback) {
  HttpClient.AjaxPost("/label/findLabelByAdimn", isAdimn, list => {
    callback(list);
  });
}
