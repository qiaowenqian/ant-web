...
export function guideUpdate(callback) {
  ...
  HttpClient.AjaxPost(
    "/user/updateLoginState?type=pc&version=" + version,
    "",
    list => {
      callback(list);
    }
...
export function dingLoginByQrCode(userCode, corpId, callback) {
  HttpClient.AjaxPost(
    "/user/dingtalkId?cid=" + userCode + "&s=" + corpId,
    "",
    list => {
      callback(list);
    }
...
export function dingLoginByCode(userCode, corpId, callback) {
  HttpClient.AjaxPost(
    "/user/dingtalkCodeLogin?code=" + userCode + "&corpid=" + corpId,
    "",
    list => {
      callback(list);
    }
...
//切换为专业版试用
export function probationOrder(buycorpid, callback) {
  HttpClient.AjaxPost(
    "/dingtalkOrder/probationOrder",
    { buycorpid: buycorpid },
    list => {
      callback(list);
    }
...
//更新试用时间
export function updateStopTime(buycorpid, callback) {
  HttpClient.AjaxPost(
    "/dingtalkOrder/updateStopTime",
    { buycorpid: buycorpid },
    list => {
      callback(list);
    }
